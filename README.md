...
- **The Feel Library** (`/feel/`) — our signature wedge: the metronome can groove in *named, real-record human feels* (J Dilla, swing, the Purdie shuffle, boom-bap, behind the beat, on top) instead of an abstract "swing %" slider. Each feel is a micro-timing pattern in `assets/js/feels.js` (single source of truth) — the data drives the playable selector, the per-feel SEO landing pages (generated by `scripts/build-feel-pages.mjs`), and the social cards (`scripts/build-feel-og.mjs`). Adding/editing a feel = edit `feels.js`, re-run both scripts.
- **Online Metronome** (`/metronome/`) — accurate Web Audio metronome: adjustable BPM (40–240), time signatures, downbeat accent, tap tempo, keyboard control, and the Feel selector. Accepts `?feel=ID&bpm=NNN&sig=N` to deep-link a feel + tempo (shareable state).
